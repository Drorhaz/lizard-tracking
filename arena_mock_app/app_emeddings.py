                ...
                if DRAW_UTILS_AVAILABLE:
                    try:
                        ...
                        if boxes is not None and len(boxes) > 0:
                            ...
                            # Add angle and direction overlays if enabled
                            if self.config.enable_angle_tracking and self.config.show_angle_overlay and self.smoothed_angle is not None:
                                ...
                                cv2.putText(frame, angle_text, (int(x1), int(y1-40)), 
                                          cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                            
                            # Draw head direction arrow if enabled
                            if (self.config.enable_angle_tracking and self.config.show_head_direction_arrow 
                                and nose and self.head_direction):
                                arrow_length = 50
                                arrow_end = (
                                    int(nose[0] + self.head_direction[0] * arrow_length),
                                    int(nose[1] + self.head_direction[1] * arrow_length)
                                )
                                cv2.arrowedLine(frame, (int(nose[0]), int(nose[1])), arrow_end, 
